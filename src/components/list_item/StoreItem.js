...
export default function StoreItem({ store: { id, addresses: [ address ], user: { name, photo }, sub_category } }) {
  return (
    <Link to={`/store/${id}`} className="flex gap-2 mb-4 bg-color hover:bg-color-gray-h md:block md:shadow">
      ...
      <div className="flex-grow md:p-2">
        ...
        <div className="text-color-gray mb-2 flex items-start gap-1">
          ...
          <span className="inline-block text-sm">{ `${address.street}, ${address.city}, ${address.state}` }</span>
        </div>
