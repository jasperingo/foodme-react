...
export default function StoreItem({ store: { id, logo, name, address, ratings } }) {
  return (
    <Link to={`/store/${id}`} className="flex mb-5 bg-color hover:bg-color-gray-h dark:hover:bg-color-gray-dh md:block">
      <img src={ `/photos/${logo}` } alt={name} className="w-20 h-20 border md:w-full md:h-full" />
